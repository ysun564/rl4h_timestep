...
processed_dir = './data/process/processed_files_old'
final_save_dir = f'./data/result/0.5h_old/sepsis_final_data_withTimes_{str(ADJTIMESTEP)}hour.csv'
final_raw_save_dir = f'./data/result/0.5h_old/sepsis_final_data_RAW_withTimes_{str(ADJTIMESTEP)}hour.csv'

print('Loading processed files created from database using "preprocess.py"')
abx           = pd.read_csv(os.path.join(processed_dir, 'abx.csv'),           sep='|')
culture       = pd.read_csv(os.path.join(processed_dir, 'culture.csv'),       sep='|')
microbio      = pd.read_csv(os.path.join(processed_dir, 'microbio.csv'),      sep='|')
demog         = pd.read_csv(os.path.join(processed_dir, 'demog.csv'),         sep='|')
ce010         = pd.read_csv(os.path.join(processed_dir, 'ce010000.csv'),      sep='|')
ce1020        = pd.read_csv(os.path.join(processed_dir, 'ce1000020000.csv'),  sep='|')
ce2030        = pd.read_csv(os.path.join(processed_dir, 'ce2000030000.csv'),  sep='|')
ce3040        = pd.read_csv(os.path.join(processed_dir, 'ce3000040000.csv'),  sep='|')
ce4050        = pd.read_csv(os.path.join(processed_dir, 'ce4000050000.csv'),  sep='|')
ce5060        = pd.read_csv(os.path.join(processed_dir, 'ce5000060000.csv'),  sep='|')
ce6070        = pd.read_csv(os.path.join(processed_dir, 'ce6000070000.csv'),  sep='|')
ce7080        = pd.read_csv(os.path.join(processed_dir, 'ce7000080000.csv'),  sep='|')
ce8090        = pd.read_csv(os.path.join(processed_dir, 'ce8000090000.csv'),  sep='|')
ce90100       = pd.read_csv(os.path.join(processed_dir, 'ce90000100000.csv'), sep='|')
MV            = pd.read_csv(os.path.join(processed_dir, 'mechvent.csv'),      sep='|')
inputpreadm   = pd.read_csv(os.path.join(processed_dir, 'preadm_fluid.csv'),  sep='|')
inputMV       = pd.read_csv(os.path.join(processed_dir, 'fluid_mv.csv'),      sep='|')
inputCV       = pd.read_csv(os.path.join(processed_dir, 'fluid_cv.csv'),      sep='|')
vasoMV        = pd.read_csv(os.path.join(processed_dir, 'vaso_mv.csv'),       sep='|')
vasoCV        = pd.read_csv(os.path.join(processed_dir, 'vaso_cv.csv'),       sep='|')
UOpreadm      = pd.read_csv(os.path.join(processed_dir, 'preadm_uo.csv'),     sep='|')
UO            = pd.read_csv(os.path.join(processed_dir, 'uo.csv'),            sep='|')
labU = [
    pd.read_csv(os.path.join(processed_dir, 'labs_ce.csv'), sep='|'),
    pd.read_csv(os.path.join(processed_dir, 'labs_le.csv'), sep='|')
]
...
Reflabs = pd.read_csv("data/ReferenceFiles/Reflabs.tsv", sep = '\t', header=None)
...
Refvitals = pd.read_csv("data/ReferenceFiles/Refvitals.tsv", sep = '\t', header=None)
...
reformat = np.nan*np.ones((2000000,68))  # Final table # NOTE: should it be adjusted due to growing data sizes in finer time steps?
qstime = dict()
...
sample_and_hold = pd.read_csv('data/ReferenceFiles/sample_and_hold.csv', index_col = None)
...
timestep = ADJTIMESTEP  # Resolution of timesteps, in hours
irow = 0   
...
for i in range(len(icustayidlist)):
    ...
    for j in np.arange(0, 79, timestep): #0:timestep:79 #-28 until +52 = 80 hours in total
        ...
        if sum(ii)>0:
            ...
            totvol = np.nansum([totvol, infu, bolus])
            reformat2[irow, 80] = totvol    #total fluid given #TODO:keep it
...
for i in range(len(icustayidlist)):
    ...
    for j in np.arange(0, 79, timestep): # 0:timestep:79 % -28 until +52 = 80 hours in total (4 hours buffer on either side of our desired window)
        t0 = 3600*j + beg   # left limit of time window
        t1 = 3600*(j + timestep) + beg   # right limit of time window
        ii = (temp[:,2] >= t0) & (temp[:,2] <= t1)  # index of items in this time period
        if sum(ii)>0:
...
df.to_csv(final_save_dir, index=False)
...
if pargs.process_raw: # If we want to convert the MIMICraw data into trajectories, repeat the above code without normalizing
    ...
    for i in data['traj'].keys():
        ...
        for ctr in range(data['traj'][i]['actions'].shape[0]):
            ...
            all_data[-1].append(data['traj'][i]['rewards'][ctr])
    df = pd.DataFrame(all_data, columns=col_names)
    df.to_csv(final_raw_save_dir, index=False)
