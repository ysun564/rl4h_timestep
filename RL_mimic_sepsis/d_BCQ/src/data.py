...
class EpisodicBuffer(Dataset):
    """Stores data as complete, sequential trajectories."""
...
class SASRBuffer(Dataset):
    """Replay buffer that stores data as a flat collection of individual transitions."""
    def __init__(self, state_dim, num_actions, buffer_size=0):
        ...
        # Add pibs of the next state. 
        self.next_pibs = torch.zeros((self.max_size, num_actions))
    ...
    def load(self, filename):
        data = torch.load(filename, map_location=torch.device('cpu'))
        state, action, reward, not_done, pibs, next_state, next_pibs = [], [], [], [], [], [], []
        for i in range(len(data['statevecs'])):
            ...
            next_pibs.append(data['pibs'][i, 1:lng, :])
        ...
        self.next_pibs = torch.cat(next_pibs)   
