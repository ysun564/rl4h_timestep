...
read_dir = (rf'F:\time_step\OfflineRL_FactoredActions\RL_mimic_sepsis\data'
            fr'\data_asNormThreshold_dt{timestep}h')
save_dir = read_dir

acuity_file = f'acuity_scores_dt{timestep}h.csv'

data_file = os.path.join(read_dir, f'sepsis_final_data_RAW_withTimes_dt{timestep}h.csv')
...
# Also keep m:icustayid for split.
keeping = table[['traj', 'm:icustayid', 'step', 'o:SOFA', 'c:oasis', 'c:sapsii']]
keeping = keeping.rename(columns={"o:SOFA": "c:SOFA", "c:oasis": "c:OASIS", "c:sapsii":"c:SAPSii"})

# Save off the acuity scores for use later 
keeping.to_csv(os.path.join(save_dir, acuity_file), index=False)
